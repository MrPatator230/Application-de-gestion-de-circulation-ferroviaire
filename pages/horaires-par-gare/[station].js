...
export default function StationSchedule() {
  ...
  const getAllStations = (schedule) => {
    ...
    const shouldShowTrackForTerminal = (time) => {
    ...
    const shouldShowTrackForIntermediate = (time) => {
      if (!time) return false;
      const scheduledTime = new Date();
      const [hours, minutes] = time.split(':').map(Number);
      scheduledTime.setHours(hours, minutes, 0);
      ...
      return minutesDiff <= 720 && minutesDiff >= -30;
    };

    // Departure station
    const departureTrack = shouldShowTrackForTerminal(schedule.departureTime) ? 
      trackAssignments[schedule.id]?.[schedule.departureStation] || '-' : 
      '-';
    stations.push({
    ...
    if (schedule.servedStations) {
      schedule.servedStations.forEach(station => {
        ...
        const track = shouldShowTrackForIntermediate(stationTime) ?
          trackAssignments[schedule.id]?.[stationName] || '-' :
          '-';
        stations.push({
          name: stationName,
    ...
    // Arrival station
    const arrivalTrack = shouldShowTrackForTerminal(schedule.arrivalTime) ?
      trackAssignments[schedule.id]?.[schedule.arrivalStation] || '-' :
      '-';
    stations.push({
...
<div key={schedule.id} className={`schedule-item ${status.className}`}>
  ...
  >
    ...
    <div className="schedule-track">
      Quai: {(() => {
        const time = scheduleType === 'departures' ? schedule.departureTime : schedule.arrivalTime;
        if (!time) return '-';
        ...
        if (minutesDiff <= 720 && minutesDiff >= -30) {
          return trackAssignments[schedule.id]?.[decodeURIComponent(station)] || '-';
        }
        return '-';
      })()}
    </div>
