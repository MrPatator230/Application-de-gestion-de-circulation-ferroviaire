                       ...
                       <div className="offer-body" style={train.isCancelled ? { textDecoration: 'line-through', color: 'red' } : {}}>
                         ...
                         <div className="quai-display mt-2">
                           Quai: {(() => {
                             ...
                             const [hours, minutes] = train.displayDepartureTime.split(':').map(Number);
                             ...
                             // Only show track if we're within 30 minutes of departure for terminal stations
                             if (train.displayDepartureStation === train.departureStation || train.displayDepartureStation === train.arrivalStation) {
                               if (minutesDiff <= 30 && minutesDiff >= -30) {
                                 return trackAssignments[train.id]?.[train.displayDepartureStation] || '-';
                               }
                             } else {
                               // 12 hours for intermediate stations
                               if (minutesDiff <= 720 && minutesDiff >= -30) {
                                 return trackAssignments[train.id]?.[train.displayDepartureStation] || '-';
                               }
                             }
                             return '-';
                ...
                <div className={modalStyles.stationsModalContent}>
                  ...
                  <div className={modalStyles.stationsGrid}>
                    ...
                    <div className={modalStyles.stationRow}>
                      ...
                      <div className={modalStyles.stationTrack}>
                        {(() => {
                          ...
                          // Only show track if we're within 30 minutes for departure station
                          if (minutesDiff <= 30 && minutesDiff >= -30) {
                            return trackAssignments[selectedSchedule.id]?.[selectedSchedule.departureStation] || '-';
                          }
                          return '-';
                        })()}
                      </div>
                    ...
                    {selectedSchedule.servedStations && selectedSchedule.servedStations.map((station, index) => {
                      ...
                      <div className={modalStyles.stationTrack}>
                        {(() => {
                          if (!station.departureTime) return '-';
                          
                          const scheduledTime = new Date();
                          const [hours, minutes] = station.departureTime.split(':').map(Number);
                          scheduledTime.setHours(hours, minutes, 0);
                          ...
                          // 12 hours for intermediate stations
                          if (stationName !== selectedSchedule.departureStation && stationName !== selectedSchedule.arrivalStation) {
                            if (minutesDiff <= 720 && minutesDiff >= -30) {
                              return trackAssignments[selectedSchedule.id]?.[stationName] || '-';
                            }
                          }
                          return '-';
                        })()}
                    ...
                    <div className={modalStyles.stationRow}>
                      ...
                      <div className={modalStyles.stationTrack}>
                        {(() => {
                          const scheduledTime = new Date();
                          const [hours, minutes] = selectedSchedule.arrivalTime.split(':').map(Number);
                          scheduledTime.setHours(hours, minutes, 0);
                          ...
                          // Only show track if we're within 30 minutes for arrival station
                          if (minutesDiff <= 30 && minutesDiff >= -30) {
                            return trackAssignments[selectedSchedule.id]?.[selectedSchedule.arrivalStation] || '-';
                          }
                          return '-';
                        })()}
                      </div>
